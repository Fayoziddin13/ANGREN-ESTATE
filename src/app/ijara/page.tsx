...
import { List, ChevronDown, Map, Layers, KeyRound } from "lucide-react";
...
export default function RentPage() {
  ...
  // Active pool of rent properties: strictly canonical published properties with transaction_type === "rent"
  const rentPropertiesPool = useMemo(() => {
  ...
  useEffect(() => {
    ...
    trackEvent("map_view", { metadata: { page: "ijara", mode: "standard" } });
  }, []);
  ...
  const filteredProperties = useMemo(() => {
    return rentPropertiesPool.filter((p) => {
      // Search Query Filter
      if (searchQuery.trim()) {
  ...
  }, [rentPropertiesPool, searchQuery, selectedDistrict, selectedType, advancedFilters]);
  ...
  const scrollToPopular = () => {
    const el = document.getElementById("rent-offers-section");
    if (el) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white overflow-x-hidden">
        ...
        <div className="mx-auto max-w-7xl flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xs sm:text-sm font-extrabold text-white">
              {locale === "uz" ? "Angrenda ijara ko‘chmas mulk" : "Аренда недвижимости в Ангрене"}
            </h1>
          </div>
          <div className="flex items-center gap-2">
      ...
      <main className="relative w-full h-[calc(100vh-104px)] sm:h-[calc(100vh-120px)] overflow-hidden bg-gray-100">
        ...
        <div className="absolute top-3 sm:top-4 left-3 right-3 sm:left-6 sm:right-auto z-20 max-w-4xl pointer-events-none">
          <FloatingSearchPanel
            ...
            onDistrictChange={(d) => {
              ...
              trackEvent("filter_used", { metadata: { district: d, page: "ijara" } });
            }}
            selectedType={selectedType}
            onTypeChange={(tp) => {
              setSelectedType(tp);
              trackEvent("filter_used", { metadata: { property_type: tp, page: "ijara" } });
            }}
            priceFilter={priceFilter}
            onPriceFilterChange={(pf) => {
              setPriceFilter(pf);
              trackEvent("filter_used", { metadata: { price: pf, page: "ijara" } });
            }}
            ...
            onAdvancedFiltersChange={(af) => {
              ...
              trackEvent("filter_used", { metadata: { advanced_filters: true, page: "ijara" } });
            }}
        ...
        >
          <span>{locale === "uz" ? "Ijara takliflari ro‘yxati" : "Предложения аренды"}</span>
