...
function getFeatureIcon(iconName: string) {
  switch (iconName) {
    ...
    case "DoorClosed":
      return <DoorClosed className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-teal-600" />;
    case "Wind":
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-2.5">
              {items.map((item) => (
                <div
                  ...
                  className="flex items-center gap-2.5 p-2.5 sm:p-3 rounded-2xl bg-white border border-slate-100/90 shadow-2xs hover:border-emerald-200 transition-all min-w-0"
                >
