...
export function PropertyCard({ property, onViewDetails }: PropertyCardProps) {
  ...
  return (
    <div
      ...
      className="group relative flex flex-col overflow-hidden rounded-2xl bg-white border border-[#dee8e3] shadow-card hover:shadow-elevated transition-all duration-200 cursor-pointer"
    >
      ...
      >
        ...
        <div className="absolute top-2.5 left-2.5 z-10 flex flex-wrap gap-1 items-center max-w-[85%]">
          <span className="rounded-lg bg-[#167d4f] px-2 py-0.5 text-[10px] sm:text-[11px] font-bold tracking-wide text-white shadow-xs">
            {badgeText}
          </span>
          {/* Automatic "New" / "Новинка" Badge (3 days from publication) */}
          {isPropertyNew(property) && (
            <span className="rounded-lg bg-[#145d3c] px-2 py-0.5 text-[10px] sm:text-[11px] font-bold tracking-wide text-white shadow-xs">
            ...
            property.badges.filter((b) => b !== "new").slice(0, 2).map((b) => {
              ...
              if (b === "arzon") {
                return (
                  <span
                    key={b}
                    className="rounded-lg bg-[#145d3c] px-2 py-0.5 text-[10px] sm:text-[11px] font-bold tracking-wide text-white shadow-xs"
                  >
              ...
              if (b === "hamyonbop" || b === "yaxshi_taklif") {
                return (
                  <span
                    key={b}
                    className="rounded-lg bg-[#167d4f] px-2 py-0.5 text-[10px] sm:text-[11px] font-bold tracking-wide text-white shadow-xs flex items-center gap-1"
                  >
        ...
        <div className="absolute top-2.5 sm:top-3.5 right-2.5 sm:right-3.5 z-10 flex items-center gap-1.5">
          ...
          <button
            ...
            className={`flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-full backdrop-blur-md shadow-sm hover:scale-110 active:scale-95 transition-all ${
              isCompared
                ? "bg-[#167d4f] text-white ring-2 ring-white/50"
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4 justify-between">
        ...
        <div className="space-y-1">
          ...
          >
            <h3 className="text-xs sm:text-sm font-bold text-gray-900 line-clamp-1 group-hover:text-[#167d4f] transition-colors">
              {title}
            </h3>
          </Link>
        ...
        {/* Specs & Action Row */}
        <div className="mt-2.5 sm:mt-3 flex items-center justify-between border-t border-[#dee8e3] pt-2 sm:pt-2.5 text-[10.5px] sm:text-xs font-semibold text-gray-600">
          ...
          <Link
            ...
            className="hidden sm:flex h-7 w-7 sm:h-8 sm:w-8 shrink-0 items-center justify-center rounded-full bg-gray-100 text-gray-700 group-hover:bg-[#167d4f] group-hover:text-white transition-colors"
          >
