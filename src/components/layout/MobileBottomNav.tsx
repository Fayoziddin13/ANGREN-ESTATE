...
export function MobileBottomNav({ onSearchClick }: MobileBottomNavProps) {
  ...
  return (
    <>
      <div className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#167d4f]/90 backdrop-blur-2xl rounded-t-2xl shadow-[0_-8px_32px_rgba(0,0,0,0.25)] border-t border-white/15">
        <nav className="flex items-center justify-around px-3 pt-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          ...
          >
            ...
            {favoritesCount > 0 && (
              <span className="absolute -top-0.5 right-1.5 flex h-3.5 min-w-3.5 px-1 items-center justify-center rounded-full bg-white text-[9px] font-black text-[#167d4f] shadow-sm">
                {favoritesCount}
              </span>
            )}
          </Link>
      ...
      {user && (
        ...
        >
          <div className="space-y-4 pt-1">
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-[#fbfcfb] border border-[#dee8e3]">
              <div className="h-11 w-11 rounded-2xl bg-[#167d4f] text-white flex items-center justify-center font-bold text-base shadow-sm">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-1 text-[11px] text-[#167d4f] font-semibold mt-0.5">
            ...
            <div className="space-y-2">
              ...
              >
                <div className="flex items-center gap-2.5 text-xs font-bold text-gray-800">
                  <Bookmark className="h-4 w-4 text-[#167d4f]" />
                  <span>{locale === "uz" ? "Saqlangan qidiruvlar" : "Сохранённые поиски"}</span>
                </div>
                {unreadNotificationsCount > 0 && (
                  <span className="px-2 py-0.5 rounded-full bg-[#167d4f] text-[10px] font-bold text-white">
              ...
              >
                <div className="flex items-center gap-2.5 text-xs font-bold text-gray-800">
                  <Scale className="h-4 w-4 text-[#167d4f]" />
